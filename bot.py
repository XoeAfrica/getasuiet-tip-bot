...
print("Starting GetASUiet Tip Bot - Final Stable Version... 💙☔️")
...
print("🤖 GetASUiet Tip Bot is running FINAL STABLE! 💙☔️🪙🍭")
    ...
    try:
        ...
        if BOT_USER_ID:
            try:
                ...
                print(f"❌ X API Error: {str(api_err)[:100]} - Expected until credits/permissions are fixed.")
        ...
        if response and hasattr(response, 'data') and response.data:
            for tweet in reversed(response.data):
                ...
                if match:
                    ...
                    if amount > 0:
                        reply = f"🎁🎉{amount}SUI tipped to @{recipient_handle} (3% maintenance fee applied) #GetASuiet🍭. Thank you for tipping."

                        try:
                            client.create_tweet(text=reply, in_reply_to_tweet_id=tid)
                            print(f"✅ Reply posted for {amount} SUI to @{recipient_handle}")
        ...
        print(f"Main loop error (caught): {str(e)[:100]}")
        time.sleep(30)
